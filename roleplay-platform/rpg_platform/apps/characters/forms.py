...
class CharacterForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "list_description": forms.Textarea(attrs={"rows": 3}),
            "content_preferences": forms.Textarea(attrs={"rows": 3}),
            "current_status": forms.Select(choices=Character.STATUS_CHOICES),
            "current_mood": forms.Select(choices=Character.MOOD_CHOICES),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["public"].help_text = _(
            "If checked, this character will be visible to other users"
        )
        self.fields["allow_random_rp"].help_text = _(
            "Allow others to invite this character to roleplay"
        )
        self.fields["show_in_lists"].help_text = _(
            "Show this character in character lists"
        )
        self.fields["list_order"].help_text = _(
            "Order in which this character appears in lists (lower numbers appear first)"
        )
        self.fields["list_description"].help_text = _(
            "Short description shown in character lists"
        )
        self.fields["content_preferences"].help_text = _(
            "Describe your character's content preferences and limits"
        )
        ...
        for field_name, field in self.fields.items():
            if (
                ...
                or isinstance(field.widget, forms.Textarea)
                or isinstance(field.widget, forms.FileInput)
            ):
    ...
    def clean(self):
        ...
        # Validate list-related fields
        if cleaned_data.get("show_in_lists"):
            if not cleaned_data.get("list_description"):
                self.add_error(
                    "list_description",
                    _("List description is required when showing in lists"),
                )
            if cleaned_data.get("list_order") is None:
                self.add_error(
                    "list_order", _("List order is required when showing in lists")
                )
...
class MultipleImageUploadForm(forms.Form):
    ...
    def clean_images(self):
        ...
        if self.files:
            ...
            if len(files) > 10:
                raise forms.ValidationError(
                    _("You can upload up to 10 images at a time.")
                )
            ...
            for image in files:
                ...
                if image.size > 5 * 1024 * 1024:
                    raise forms.ValidationError(
                        _("Image '%(name)s' is too large. Maximum size is 5MB.")
                        % {"name": image.name}
                    )
...
class KinkBulkForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if character:
            ...
            kinks = Kink.objects.select_related("category").order_by(
...
class CharacterSearchForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                "class": "form-control",
                "placeholder": _("Search character name, species, etc..."),
            }
        ),
    )
    ...
    gender = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("Filter by gender")}
        ),
    )
    ...
    min_age = forms.IntegerField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control", "placeholder": _("Min age")}
        ),
    )
    ...
    max_age = forms.IntegerField(
        ...
        min_value=0,
        widget=forms.NumberInput(
            attrs={"class": "form-control", "placeholder": _("Max age")}
        ),
    )
    ...
    species = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("Filter by species")}
        ),
    )
...
class CharacterRatingForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ["rating"]
        widgets = {"rating": forms.RadioSelect(attrs={"class": "star-rating"})}
